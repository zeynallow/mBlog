  ...
  <div class="content-wrapper">
    <div class="row">
      <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
          <div class="card-body">
            <h4 class="card-title">@lang('admin.edit_page') - ID: {{$page->id}}</h4>
            <hr/>
                  ...
                  @foreach ($page->page_data_all as $key => $page_data)
                    <div id="{{$page_data->locale}}-content" class="tab-pane fade {{($key == 0) ? 'in active show' : ''}} ">
                      ...
                      <div class="form-group">
                        <label for="title_{{$page_data->locale}}">@lang('admin.title')</label>
                        <input value="{{$page_data->title}}" type="text" class="form-control" name="title[{{$page_data->locale}}]" id="title_{{$page_data->locale}}" placeholder="@lang('admin.title')">
                      </div>

                      <input value="{{$page_data->id}}" type="hidden" name="page_data_id[{{$page_data->locale}}]">
                      <div class="form-group">
                        <label for="text_{{$page_data->locale}}">@lang('admin.content')</label>
